	...
	prepareMessage (messageData, options = {}) {
		...
		if (options.checkIn) {
			...
			const embed = {
				...
				fields: [
					{
						name: `[${uid}] ${username}\'s Current Stamina`,
			...
			const embed = {
				...
				description: `⚠️ All expeditions are done! ⚠️`,
				fields: [
					{
						name: `[${uid}] ${username}`,
						value: "All expeditions are done!",
						inline: false
					}
				],
				timestamp: new Date(),
			...
			const embed = {
				...
				fields: [
					{
						name: `[${uid}] ${username}\'s Current Reserve Stamina`,
